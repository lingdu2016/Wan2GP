...
def save_lset(state, lset_name, loras_choices, loras_mult_choices, prompt, save_lset_prompt_cbox):    
    ...
    if len(lset_name) == 0:
        ...
        lset_choices =[("请输入 Lora 预设 / 设置文件的名称","")]
    else:
        lset_name = sanitize_file_name(lset_name)
...
def get_prompt_enhancer_choices(model_def, audio_only=False, image_mode=0, include_disabled=True):
    choices = [("禁用", "")] if include_disabled else []
    prompt_enhancer_default = ""
    default_labels = {
        "T": "基于文本提示词内容",
        "TI": "基于文本提示词和图片提示词内容（起始图片 / 第一张参考图片）",
    }
...
def refresh_video_guide_outpainting_labels(video_guide_outpainting_ratio):
    ...
    return gr.update(label=f"上方 {suffix}"), gr.update(label=f"下方 {suffix}"), gr.update(label=f"左侧 {suffix}"), gr.update(label=f"右侧 {suffix}")
...
def change_guidance_phases(state, guidance_phases):
    ...
    visible_phases = model_def.get("visible_phases", guidance_phases) 
    multiple_submodels = model_def.get("multiple_submodels", False)
    label ="阶段 1-2" if guidance_phases ==3 else ( "模型 / 引导切换阈值" if multiple_submodels  else "引导切换阈值" )
...
memory_profile_choices= [   ("配置 1，HighRAM_HighVRAM：至少 64 GB 内存和 24 GB 显存，使用 RTX 3090 / RTX 4090 生成短视频时速度最快", 1),
                            ("配置 2，HighRAM_LowVRAM：至少 64 GB 内存和 12 GB 显存，凭借大内存成为最通用的配置，更适合 RTX 3070/3080/4070/4080，或用于 RTX 3090 / RTX 4090 处理大批量图片或长视频", 2),
                            ("配置 3，LowRAM_HighVRAM：至少 32 GB 内存和 24 GB 显存，适合内存有限但需要短视频高速生成的 RTX 3090 / RTX 4090",3),
                            ("配置 3+，VeryLowRAM_HighVRAM：至少 32 GB 内存和 24 GB 显存，配置 3 的变体，不使用保留内存以降低内存占用",3.5),
                            ("配置 4，LowRAM_LowVRAM（推荐）：至少 32 GB 内存和 12 GB 显存，适合显存较少或想生成更长视频的用户",4),
                            ("配置 4+，LowRAM_LowVRAM+：至少 32 GB 内存和 12 GB 显存，配置 4 的变体，速度稍慢但显存需求更低",4.5),
                            ("配置 5，VerylowRAM_LowVRAM（保底方案）：至少 24 GB 内存和 10 GB 显存，配置不足时的兜底选项，速度不快但也许能跑起来",5)]

def check_attn(mode):
    if mode not in attention_modes_installed: return "（未安装）"
    if mode not in attention_modes_supported: return "（不支持）"
    return ""

attention_modes_choices= [
    ("自动：使用最佳可用模式（sage2 > sage > sdpa）", "auto"),
    ("sdpa：默认模式，始终可用", "sdpa"),
    (f'flash{check_attn("flash")}：质量高，需手动安装', "flash"),
    (f'xformers{check_attn("xformers")}：质量好，显存占用更低，需手动安装', "xformers"),
    (f'sage{check_attn("sage")}：约快 30%，需手动安装', "sage"),
    (f'sage2/sage2++{check_attn("sage2")}：约快 40%，需手动安装', "sage2"),
] + ([(f'radial{check_attn("radial")}：实验性功能，可能更快，需手动安装', "radial")] if args.betatest else []) + [
    (f'sage3{check_attn("sage3")}：快 50% 以上，可能有一定质量折衷，需手动安装', "sage3"),
]
    ...
    with gr.Row():
        ...
        with gr.Column(**column_kwargs):
            ...
            with gr.Row(visible= True): #len(loras)>0) as presets_column:
                ...
                with gr.Column(scale=1):
                    with gr.Row(height=17): 
                        apply_lset_btn = gr.Button("应用", size="sm", min_width= 1)
                        refresh_lora_btn = gr.Button("刷新", size="sm", min_width= 1, visible=advanced_ui or not only_allow_edit_in_advanced)
                        if len(launch_preset) == 0 : 
                            lset_type = 2   
                        else:
                    ...
                    with gr.Row(height=17, visible=False) as refresh2_row:
                        refresh_lora_btn2 = gr.Button("刷新", size="sm", min_width= 1)

                    with gr.Row(height=17, visible=advanced_ui or not only_allow_edit_in_advanced) as preset_buttons_rows:
                        confirm_save_lset_btn = gr.Button("确认保存！", size="sm", min_width= 1, visible=False) 
                        confirm_delete_lset_btn = gr.Button("确认删除！", size="sm", min_width= 1, visible=False) 
                        save_lset_btn = gr.Button("保存", size="sm", min_width= 1, visible = True)
                        delete_lset_btn = gr.Button("删除", size="sm", min_width= 1, visible = True)
                        cancel_lset_btn = gr.Button("算了，不要！", size="sm", min_width= 1 , visible=False)  
            ...
            with gr.Tabs(visible = v2i_switch_supported or inpaint_support, selected= image_mode_tab_selected ) as image_mode_tabs:
                with gr.Tab("文生视频", id = "t2v", elem_classes="compact_tab", visible = v2i_switch_supported) as tab_t2v:
                    pass
                with gr.Tab("文生图", id = "t2i", elem_classes="compact_tab"):
                    pass
                with gr.Tab("图片局部重绘", id = "inpaint", elem_classes="compact_tab", visible=inpaint_support) as tab_inpaint:
                    pass
            ...
            with gr.Column(visible= image_prompt_types_allowed not in ("", "T") or model_mode_choices is not None and image_mode_value in model_modes_visibility ) as image_prompt_column: 
                ...
                if "T" in image_prompt_types_allowed: 
                    image_prompt_type_choices += [("文本提示词" if "S" in image_prompt_types_allowed else "新建视频", "")]
                if "S" in image_prompt_types_allowed: 
                    image_prompt_type_choices += [("以图片开始", "S")]
                    any_start_image = True
                if "V" in image_prompt_types_allowed:
                    any_video_source = True
                    image_prompt_type_choices += [("续写视频", "V")]
                if "L" in image_prompt_types_allowed:
                    any_video_source = True
                    image_prompt_type_choices += [("续写上一段视频", "L")]
                with gr.Group(visible= len(image_prompt_types_allowed)>1 and image_mode_value == 0) as image_prompt_type_group:
                    with gr.Row():
                        ...
                        if len(image_prompt_type_choices) > 0:
                            image_prompt_type_radio = gr.Radio( image_prompt_type_choices, value = image_prompt_type_radio_value, label="位置", show_label= False, visible= len(image_prompt_types_allowed)>1, scale= 5)
                        else:
                            image_prompt_type_radio = gr.Radio(choices=[("", "")], value="", visible= False)
                        if "E" in image_prompt_types_allowed:
                            image_prompt_type_endcheckbox = gr.Checkbox( value ="E" in image_prompt_type_value, label="结束图片", show_label= False, visible=end_option_visible, scale= 1, elem_classes="cbx_centered")
                            any_end_image = True
                        else:
                            image_prompt_type_endcheckbox = gr.Checkbox( value =False, show_label= False, visible= False , scale= 1)
                image_start_row, image_start, image_start_extra = get_image_gallery(label= "作为生成队列中新视频起点的图片" + (" (None for Black Frames)" if model_def.get("black_frame", False) else ''), value = ui_defaults.get("image_start", None), visible= "S" in image_prompt_type_value )
                ...
                if model_mode_choices is None:
                    model_mode = gr.Dropdown(value=None, label="模型模式", visible=False)
                else:
                ...
                keep_frames_video_source = gr.Text(value=ui_get("keep_frames_video_source") , visible= len(filter_letters(image_prompt_type_value, "VL"))>0 , scale = 2, label= "超过该重采样帧数后截断视频（留空=保留全部，负数表示从末尾截断）" ) 
            ...
            with gr.Column(visible= guide_preprocessing is not None or mask_preprocessing is not None or guide_custom_choices is not None or image_ref_choices is not None  ) as video_prompt_column: 
                ...
                with gr.Row(visible=guide_selection_visible) as guide_selection_row:
                    ...
                    if guide_preprocessing is None:
                        video_prompt_type_video_guide = gr.Dropdown(choices=[("","")], value="", label="控制视频", scale = 2, visible= False, show_label= True, )
                    else:
                    ...
                    if guide_custom_choices is None:
                        video_prompt_type_video_guide_alt = gr.Dropdown(choices=[("","")], value="", label="控制视频", visible= False, scale = 1 )
                    else:
                    ...
                    if custom_video_selection is None:
                        video_prompt_type_video_custom_dropbox = gr.Dropdown(choices=[("","")], value="", label="自定义下拉框", scale = 1, visible= False, show_label= True, )
                        video_prompt_type_video_custom_checkbox = gr.Checkbox(value=False, label="自定义复选框", scale = 1, visible= False, show_label= True, )
                    ...
                    if mask_preprocessing is None:
                        video_prompt_type_video_mask = gr.Dropdown(choices=[("","")], value="", label="视频遮罩", scale = 1, visible= False, show_label= True, )
                    ...
                    if image_ref_inpaint:
                       image_ref_choices = { "choices": [("无", ""), ("人物 / 物体", "I"), ], "letters_filter":  "I", }
                    ...
                    if image_ref_choices is None:
                        ...
                        video_prompt_type_image_refs = gr.Dropdown(
                            ...
                            label=image_ref_choices.get("label", "注入参考图片"), show_label= image_ref_choices.get("show_label", True), scale = 1
                        )

                image_guide = gr.Image(label= "控制图片", height = 800, type ="pil", visible= image_mode_value==1 and "V" in video_prompt_type_value and ("U" in video_prompt_type_value or "A" not in video_prompt_type_value ) , value= ui_defaults.get("image_guide", None))
                ...
                if image_mode_value >= 1:  
                    ...
                    with gr.Column(elem_classes=["wangp-magic-mask-anchor", "wangp-magic-mask-anchor--image-editor"]):
                        image_mask_guide = gr.ImageEditor(
                            label="待局部重绘的控制图片" if image_mode_value == 2 else "控制图片与遮罩",
                ...
                keep_frames_video_guide = gr.Text(value=ui_get("keep_frames_video_guide") , visible= keep_frames_video_guide_visible  , scale = 2, label= "控制视频中要保留的帧（留空=全部，1=第一帧，a:b 表示范围，多个值用空格分隔）" ) #, -1=last
                ...
                with gr.Column(visible= ("V" in video_prompt_type_value  or "K" in video_prompt_type_value  or "F" in video_prompt_type_value) and image_mode_value in video_guide_outpainting_modes) as video_guide_outpainting_col:
                    ...
                    with gr.Group():
                        ...
                        with gr.Row():
                            video_guide_outpainting_checkbox = gr.Checkbox(label=model_def.get("video_guide_outpainting_label", "在控制视频、横版或定位参考帧上启用空间外扩") if image_mode_value == 0 else "在控制图片上启用空间外扩", value=video_guide_outpainting_enabled, scale=3)
                            video_guide_outpainting_ratio = gr.Dropdown([("手动扩展", ""), ("适配 1:1 画框", "1:1"), ("适配 4:3 画框", "4:3"), ("适配 3:4 画框", "3:4"), ("适配 16:9 画框", "16:9"), ("适配 9:16 画框", "9:16"), ("适配 21:9 画框", "21:9"), ("适配 9:21 画框", "9:21")], value=video_guide_outpainting_ratio_value, visible=video_guide_outpainting_enabled, show_label=False, allow_custom_value=False, scale=1)
                        with gr.Row(visible = not video_guide_outpainting_value.startswith("#")) as video_guide_outpainting_row:
                            ...
                            video_guide_outpainting_top= gr.Slider(0, 100, value= video_guide_outpainting_list[0], step=5, label=f"上方 {outpainting_label_suffix}", show_reset_button= False)
                            video_guide_outpainting_bottom = gr.Slider(0, 100, value= video_guide_outpainting_list[1], step=5, label=f"下方 {outpainting_label_suffix}", show_reset_button= False)
                            video_guide_outpainting_left = gr.Slider(0, 100, value= video_guide_outpainting_list[2], step=5, label=f"左侧 {outpainting_label_suffix}", show_reset_button= False)
                            video_guide_outpainting_right = gr.Slider(0, 100, value= video_guide_outpainting_list[3], step=5, label=f"右侧 {outpainting_label_suffix}", show_reset_button= False)
                # image_mask = gr.Image(label= "图片遮罩区域（用于局部重绘，白色 = 控制区域，黑色 = 保持不变）", type ="pil", visible= image_mode_value==1 and "V" in video_prompt_type_value and "A" in video_prompt_type_value and not "U" in video_prompt_type_value , height = gallery_height, value= ui_defaults.get("image_mask", None)) 
                image_mask = gr.Image(label= "图片遮罩区域（用于局部重绘，白色 = 控制区域，黑色 = 保持不变）", type ="pil", visible= False, height = gallery_height, value= ui_defaults.get("image_mask", None)) 
                ...
                frames_positions = gr.Text(value=ui_get("frames_positions") , visible= "F" in video_prompt_type_value, scale = 2, label= "注入帧的位置（1=第一帧，L=窗口的最后一帧，其他图片参考无需指定位置）" ) 
                ...
                background_removal_label = model_def.get("background_removal_label", "去除人物 / 物体背后的背景") 
                ...
                remove_background_images_ref = gr.Dropdown(
                    ...
                    label="自动去除参考图片中人物或物体背后的背景", scale = 3, visible= "I" in video_prompt_type_value and not no_background_removal
                )
            ...
            if any_audio_prompt:
                ...
                audio_prompt_type_sources = gr.Dropdown(
                    ...
                    label=audio_prompt_type_sources_def.get("label", "语音"),
            ...
            with gr.Row(visible = any_audio_prompt and any_letters(audio_prompt_type_sources_value,"AB") and not image_outputs) as audio_guide_row:
                ...
                audio_guide = gr.Audio(value= ui_defaults.get("audio_guide", None), type="filepath", label= model_def.get("audio_guide_label","待跟随的语音"), show_download_button= True, visible= any_audio_prompt and any_letters(audio_prompt_type_value, "A") )
                audio_guide2 = gr.Audio(value= ui_defaults.get("audio_guide2", None), type="filepath", label=model_def.get("audio_guide2_label","待跟随的语音 #2"), show_download_button= True, visible= any_audio_prompt and "B" in audio_prompt_type_value )
            custom_guide_def = model_def.get("custom_guide", None)
            ...
            with gr.Row(visible = any_custom_guide) as custom_guide_row:
                if custom_guide_def is None:
                    custom_guide = gr.File(value= None, type="filepath", label= "自定义引导", height=41, visible= False )
                else:
                    custom_guide = gr.File(value= ui_defaults.get("custom_guide", None), type="filepath", label= custom_guide_def.get("label","自定义引导"), height=41, visible= True, file_types = custom_guide_def.get("file_types", ["*.*"]) )
            ...
            with gr.Row(visible=remove_background_visible or normalize_audio_visible or custom_audio_option_visible) as audio_options_row:
                remove_background_sound = gr.Checkbox(label="去除背景音乐 / 噪音" if audio_only else "忽略背景音乐（可改善对口型效果）", value="V" in audio_prompt_type_value, visible=remove_background_visible)
                normalize_audio_volumes = gr.Checkbox(label="归一化音频音量", value="N" in audio_prompt_type_value, visible=normalize_audio_visible)
            ...
            with gr.Row(visible = any_audio_prompt and any_multi_speakers and ("B" in audio_prompt_type_value or "X" in audio_prompt_type_value) and not image_outputs ) as speakers_locations_row:
                speakers_locations = gr.Text( ui_get("speakers_locations"), label="说话人位置，以空格分隔。每个位置格式为 左:右 或 边界框 左:上:右:下", visible= True)
            ...
            with gr.Column(visible=not advanced_prompt and len(variables) > 0) as prompt_column_wizard_vars:
                gr.Markdown("<B>请填写以下输入字段，以自动适配提示词：</B>")
                wizard_prompt_activated = "off"
                wizard_variables = ""
                with gr.Row():
            ...
            with gr.Row(visible= server_config.get("enhancer_enabled", 0) > 0  ) as prompt_enhancer_row:
                ...
                prompt_enhancer_mode_dropdown = gr.Dropdown(
                    ...
                    label=model_def.get("prompt_enhancer_button_label", "使用大语言模型优化提示词") , scale = 5,
                    visible= True, show_label= not on_demand_prompt_enhancer,
                )
                prompt_enhancer_think = gr.Checkbox(label="思考", value=prompt_enhancer_think_value, visible=prompt_enhancer_think_visible, scale=1, elem_classes=prompt_enhancer_think_classes)
            alt_prompt_def = model_def.get("alt_prompt", None)
            ...
            with gr.Row(visible=not audio_only) as resolution_row:
                fit_canvas = server_config.get("fit_canvas", 0)
                if fit_canvas == 1:
                    label = "外框分辨率（为保持视频宽高比，其中一边可能会更小）"
                elif fit_canvas == 2:
                    label = "输出分辨率（若宽高比不同，输入图片将被裁剪）"
                else:
                    label = "分辨率预算（像素将被重新分配以保持输入的宽高比）" 
            ...
            with gr.Row(visible= not audio_only) as number_frames_row:
                ...
                if image_outputs:
                    video_length = gr.Slider(1, 9999, value=ui_get("video_length"), step=1, label="帧数", visible = False, show_reset_button= False)
                else:
            ...
            with gr.Row(visible = not lock_inference_steps) as inference_steps_row:                                       
                num_inference_steps = gr.Slider(0 if audio_only else 1, 100, value=ui_get("num_inference_steps"), step=1, label="推理步数", visible = True, show_reset_button= False)


            show_advanced = gr.Checkbox(label="高级模式", value=advanced_ui)
            with gr.Tabs(visible=advanced_ui) as advanced_row:
                ...
                with gr.Tab("通用"):
                    with gr.Column():
                        with gr.Row():                        
                            seed = gr.Slider(-1, 999999999, value=ui_get("seed"), step=1, label="随机种子（-1 表示随机）", scale=2, show_reset_button= False) 
                            if model_def.get("lock_guidance_phases", False):
                            ...
                            guidance_phases = gr.Dropdown(
                                choices= (["none", 0] if guidance_phases_value == 0 else []) + [("单阶段", 1),("两阶段", 2)] + ([("三阶段", 3)] if guidance_max_phases >=3 else []),
                                value= guidance_phases_value,
                                label="引导阶段数" if visible_phases>=2 else "阶段数",
                        ...
                        with gr.Row(visible=model_def.get("pause_between_sentences", False)) as pause_row:
                            pause_seconds = gr.Slider(minimum=0.0, maximum=2.0,  value=ui_get("pause_seconds"), step=0.05, label="多说话人语句之间的停顿（秒）", show_reset_button=False,)
                        ...
                        with gr.Row(visible = get_container_def("sample_solver_row").visible ) as sample_solver_row:
                            if sample_solver_choices is None:
                                sample_solver = gr.Dropdown( value="",  choices=[ ("", ""), ], visible= False, label= "采样求解器 / 调度器" )
                            else:
                                sample_solver = gr.Dropdown( value=ui_get("sample_solver", sample_solver_choices[0][1]), 
                                    choices= sample_solver_choices, visible= True, label= "采样求解器 / 调度器"
                                )
                        ...
                        with gr.Row(visible = not (hunyuan_t2v or hunyuan_i2v or no_negative_prompt)) as negative_prompt_row:
                            ...
                            negative_prompt = gr.Textbox(label="负向提示词 " + ("(ignored if NAG is disabled and no Guidance that is if CFG = 1)"  if get_container_def("NAG_col").visible else "(ignored if no Guidance that is if CFG = 1)") , value=ui_get("negative_prompt")  )
                        with gr.Column(visible = get_container_def("NAG_col").visible) as NAG_col:
                            gr.Markdown("<B>即使未设置引导（CFG = 1），NAG 也会强制生效负向提示词，将 NAG Scale 设为大于 1 即可启用</B>")
                            with gr.Row():
                        ...
                        with gr.Row():
                            repeat_generation = gr.Slider(1, 25.0, value=ui_get("repeat_generation"), step=1, label=f"每条提示词生成的{'音频文件' if audio_only else '视频'}数量", visible = not image_outputs, show_reset_button= False) 
                            multi_images_gen_type = gr.Dropdown( value=ui_get("multi_images_gen_type"), 
                                choices=[
                                    ("为图片和文本生成所有组合", 0),
                                    ("按顺序匹配图片与文本提示词", 1),
                                ], visible=multiple_images_as_text_prompts and not edit_mode, label= "多张图片作为文本提示词"
                            )
                        with gr.Row():
                ...
                with gr.Tab("LoRA", visible= not audio_only or model_def.get("enabled_audio_lora", False)) as loras_tab:
                    with gr.Column(visible = True): #as loras_column:
                        gr.Markdown("<B>LoRA 可通过在提示词中加入触发词，为视频制造特殊效果。你可以将 LoRA 组合保存为预设。</B>")
                        ...
                        loras_multipliers = gr.Textbox(label="LoRA 权重系数（默认 1.0），以空格或换行分隔，以 # 开头的行会被忽略", value=launch_multis_str)
                with gr.Tab("跳步", visible = any_tea_cache or any_mag_cache) as speed_tab:
                    with gr.Column():
                        gr.Markdown("<B>Tea Cache 和 Mag Cache 通过智能跳过部分步骤来加速视频生成，跳过的步数越多，视频质量越低。</B>")
                        gr.Markdown("<B>跳步同样会消耗显存。建议至少不要跳过前 10% 的步数。</B>")
                        steps_skipping_choices = [("无", "")]
                        ...
                        skip_steps_start_step_perc = gr.Slider(0, 100, value=ui_get("skip_steps_start_step_perc"), step=1, label="跳步在生成过程中的起始时刻百分比", show_reset_button= False) 

                with gr.Tab("后期处理", visible = not audio_only) as post_processing_tab:
                    

                    with gr.Column():
                        gr.Markdown("<B>插帧放大——一种可提升流畅度及输出尺寸的后期处理</B>")
                        def gen_upsampling_dropdowns(temporal_upsampling, spatial_upsampling , film_grain_intensity, film_grain_saturation, element_class= None, max_height= None, image_outputs = False, late_postprocessing = False):
                            ...
                            with gr.Row():
                                film_grain_intensity = gr.Slider(0, 1, value=film_grain_intensity, step=0.01, label="胶片颗粒强度（0 = 禁用）", show_reset_button= False) 
                                film_grain_saturation = gr.Slider(0.0, 1, value=film_grain_saturation, step=0.01, label="胶片颗粒饱和度", show_reset_button= False) 
                ...
                with gr.Tab("音频", visible = not (image_outputs or audio_only)) as audio_tab:
                ...
                with gr.Tab("质量", visible = (vace and image_outputs or any_perturbation or any_cfg_zero or any_cfg_star or any_apg or any_motion_amplitude or any_pnp) and not audio_only ) as quality_tab:
                        with gr.Column(visible = any_perturbation ) as perturbation_row:
                            gr.Markdown("<B>扰动（可提升视频质量，需要引导系数大于 1）</B>")
                            perturbation_choices = model_def.get("perturbation_choices", [("关闭", 0), ("跳层引导", 1)])
                            ...
                            with gr.Row():
                                perturbation_start_perc = gr.Slider(0, 100, value=ui_get("perturbation_start_perc"), step=1, label="去噪步数百分比（开始）", show_reset_button= False)
                                perturbation_end_perc = gr.Slider(0, 100, value=ui_get("perturbation_end_perc"), step=1, label="去噪步数百分比（结束）", show_reset_button= False)

                        with gr.Column(visible= any_apg ) as apg_col:
                            gr.Markdown("<B>修正长视频生成过程中的渐进式色彩饱和度问题")
                            apg_switch = gr.Dropdown(
                                ...
                                visible=True,
                                scale = 1,
                                label="自适应投影引导（需要引导系数 > 1 或音频引导系数 > 1） " if multitalk else "自适应投影引导（需要引导系数 > 1）",
                            )

                        with gr.Column(visible = any_cfg_star) as cfg_free_guidance_col:
                            gr.Markdown("<B>无分类器引导零星（CFG Zero Star），可更好地贴合文本提示词")
                            cfg_star_switch = gr.Dropdown(
                                ...
                                visible=True,
                                scale = 1,
                                label="无分类器引导之星（CFG Star，需要引导系数 > 1）"
                            )
                            with gr.Row():
                                cfg_zero_step = gr.Slider(-1, 39, value=ui_get("cfg_zero_step"), step=1, label="低于该层使用 CFG Zero（额外处理）", visible = any_cfg_zero, show_reset_button= False) 

                        with gr.Column(visible = v2i_switch_supported and image_outputs) as min_frames_if_references_col:
                            gr.Markdown("<B>仅生成单帧可能不足以保留参考图片的身份信息 / 控制图片信息，也难以获得良好的图片质量。一种变通方法是生成一小段视频并保留第一帧。")
                            ...
                            image_mode_video_frame_choices = [("禁用，仅生成一帧", 1)]
                            image_mode_video_frame_choices += [(f"仅当存在参考图片 / 控制图片时生成 {frame_count} 帧的视频（慢 {speed}）", frame_count) for frame_count, speed in zip(frame_counts, speed_labels)]
                            image_mode_video_frame_choices += [(f"始终生成 {frame_count} 帧的视频（慢 {speed}）", 1000 + frame_count) for frame_count, speed in zip(frame_counts, speed_labels)]
                        ...
                        with gr.Column(visible = model_def.get("self_refiner", False)) as self_refiner_col:
                            gr.Markdown("<B>自精炼视频采样（PnP）——可提升运动质量</B>")
                            self_refiner_setting = gr.Dropdown(choices=[("禁用", 0),("启用（P1-Norm）", 1), ("启用（P2-Norm）", 2)], value=ui_get("self_refiner_setting", 0), scale=1, label="自精炼器")
                            ...
                            with gr.Column(visible=(update_form and ui_get("self_refiner_setting", 0) > 0)) as self_refiner_rules_ui:
                                ...
                                with gr.Row(elem_id="refiner-input-row"):
                                    if RangeSlider is not None:
                                        refiner_range = RangeSlider(minimum=1, maximum=100, value=(1, 10), step=1, label="步数范围", info="开始 - 结束", scale=3)
                                    else:
                                        refiner_range = gr.Textbox(value="1-10", label="步数范围", info="开始-结束", scale=3)
                                    refiner_mult = gr.Slider(label="迭代次数", value=3, minimum=1, maximum=5, step=1, scale=2)
                                    refiner_add_btn = gr.Button("➕ 添加", variant="primary", scale=0, min_width=100)
                                ...
                                if not update_form:
                                    ...
                                    def render_refiner_rules(rules):
                                        if not rules:
                                            gr.Markdown("<I style='padding: 8px;'>未定义规则。使用默认值：步数 2-5（3 倍），步数 6-13（1 倍）。</I>")
                                            return
                            ...
                            with gr.Row():
                                self_refiner_f_uncertainty = gr.Slider(0.0, 1.0, value=ui_get("self_refiner_f_uncertainty", 0.0), step=0.01, label="不确定性阈值", show_reset_button= False)
                                self_refiner_certain_percentage = gr.Slider(0.0, 1.0, value=ui_get("self_refiner_certain_percentage", 0.999), step=0.001, label="确定性百分比跳过", show_reset_button= False)
                            

                with gr.Tab("滑动窗口", visible= sliding_window_enabled and not image_outputs and not audio_only) as sliding_window_tab:

                    with gr.Column():  
                        gr.Markdown("<B>滑动窗口允许你生成不受模型限制时长的视频</B>")
                        gr.Markdown("<B>当要生成的帧数超过窗口大小时，会自动开启该功能</B>")
                        if diffusion_forcing:
                            sliding_window_size = setting_slider("sliding_window_size", minimum=37, maximum=get_max_frames(257), value=ui_defaults.get("sliding_window_size", 129), step=20, label="  （建议保持为 97）", visible=True, respect_setting_visibility=False)
                ...
                with gr.Tab("杂项") as misc_tab:
                    with gr.Column(visible = not (recammaster or ltxv or diffusion_forcing or audio_only or image_outputs)) as RIFLEx_setting_col:
                        ...
                        RIFLEx_setting = gr.Dropdown(
                            ...
                            label="用于生成长视频的 RIFLEx 位置编码",
                            visible = True
                        )
                    with gr.Column(visible = not (audio_only or image_outputs)) as force_fps_col:
                        gr.Markdown("<B>你可以更改输出视频的默认帧率，在没有控制视频的情况下，这可能会造成不必要的慢放/快进</B>")
                        force_fps_choices =  [(f"模型默认值（{fps} fps）", "")]
                        if any_control_video and (any_video_source or recammaster):
                            force_fps_choices +=  [("自动帧率：优先源视频，其次控制视频，否则模型默认值", "auto")]
                        elif any_control_video :
                            force_fps_choices +=  [("自动帧率：控制视频（如有），否则模型默认值", "auto")]
                        elif any_control_video and (any_video_source or recammaster):
                            force_fps_choices +=  [("自动帧率：源视频（如有），否则模型默认值", "auto")]
                        if any_control_video:
                            force_fps_choices +=  [("控制视频帧率", "control")]
                        if any_video_source or recammaster:
                            force_fps_choices +=  [("源视频帧率", "source")]
                        force_fps_choices += [
                                ("15", "15"), 
                                ("16", "16"), 
                                ("23", "23"), 
                        ...
                        force_fps = gr.Dropdown(
                            ...
                            label=f"覆盖帧率（模型默认值={fps} fps）"
                        )

                    profile_type = get_profile_type_for_model(base_model_type, image_mode_value)
                    profile_type = profile_type[0].upper() + profile_type[1:]
                    gr.Markdown("<B>如果你只生成短视频或图片，可以设置更激进的内存配置<B>")
                    override_profile = gr.Dropdown(
                        choices=[(f"默认 {profile_type} 内存配置", -1)] + memory_profile_choices,
                        value=ui_get("override_profile"),
                        label=f"覆盖内存配置"
                    )

                    gr.Markdown("<B>你可以设置不同的注意力模式以提升质量/兼容性<B>")
                    override_attention = gr.Dropdown(
                        choices=[("默认注意力模式", "")] + attention_modes_choices,
                        value=ui_get("override_attention"),
                        label=f"覆盖注意力模式"
                    )

                    with gr.Column():
                        gr.Markdown('<B>使用设置中的值自定义输出文件名（<I>date、seed、resolution、num_inference_steps、prompt、flow_shift、video_length、guidance_scale</I>）。例如：<BR>"<I>{date(YYYY-MM-DD_HH-mm-ss)}_{seed}_{prompt(50)}, {num_inference_steps}</I>"</B>')
                        output_filename = gr.Text( label= " 输出文件名（留空则自动命名）", value= ui_get("output_filename"))

            if not update_form:
                with gr.Row(visible=(tab_id == 'edit')):
                    edit_btn = gr.Button("应用修改", elem_id="edit_tab_apply_button")
                    cancel_btn = gr.Button("取消", elem_id="edit_tab_cancel_button")
                    silent_cancel_btn = gr.Button("静默取消", elem_id="silent_edit_tab_cancel_button", visible=False)
            with gr.Column(visible= not edit_mode):
                with gr.Row():
                    save_settings_btn = gr.Button("将设置设为默认", visible = not args.lock_config)
                    export_settings_from_file_btn = gr.Button("导出设置到文件")
                    export_settings_include_media = gr.Checkbox(label="包含媒体文件", value=False)
                    reset_settings_btn = gr.Button("重置设置")
                with gr.Row():
                    settings_file = gr.File(height=41,label="从媒体文件 / Json / Zip 加载设置")
                    settings_download_payload = gr.Text(interactive=False, visible=False, value="")
                with gr.Group():
                    with gr.Row():
                        lora_url = gr.Text(label ="Lora 链接", placeholder= "输入 Lora 链接", scale=4, show_label=False, elem_classes="compact_text" )
                        download_lora_btn = gr.Button("下载 Lora", scale=1, min_width=10)
        ...
        with gr.Column(visible=(tab_id == 'generate')):
            if not update_form:
                ...
                gen_status = gr.Text(interactive=False, label="状态", lines=1, max_lines=1, autoscroll=False)
                ...
                with gr.Tabs() as gallery_tabs:
                    with gr.Tab("视频 / 图片库", id="video_images"):
                        output = gr.Gallery(value =default_files, label="已生成的视频", preview= True, show_label=False, elem_id="gallery" , columns=[3], rows=[1], object_fit="contain", height=450, selected_index=0, interactive= False)
                    with gr.Tab("音频文件库", id="audio"):
            ...
            with gr.Accordion("媒体信息 / 后期处理 / 导入媒体", open=False) as video_info_accordion:
                ...
                with gr.Tabs() as video_info_tabs:
                    ...
                    with gr.Tab("信息", id="video_info"):
                        ...
                        with gr.Row(**default_visibility_false) as audio_buttons_row:
                            video_info_extract_audio_settings_btn = gr.Button("提取设置", min_width= 1, size ="sm")
                            video_info_to_audio_guide_btn = gr.Button("设为音频源", min_width= 1, size ="sm", visible = any_audio_guide)
                            video_info_to_audio_guide2_btn = gr.Button("设为音频源 2", min_width= 1, size ="sm", visible = any_audio_guide2 )
                            video_info_to_audio_source_btn = gr.Button("设为配乐", min_width= 1, size ="sm", visible=audio_processor_api.has_audio_source_soundtrack())
                            video_info_eject_audio_btn = gr.Button("弹出音频文件", min_width= 1, size ="sm")
                        with gr.Row(**default_visibility_false) as deleted_audio_buttons_row:
                            video_info_eject_deleted_audio_btn = gr.Button("弹出已删除文件", min_width= 1, size ="sm")
                        with gr.Row(**default_visibility_false) as video_buttons_row:
                            video_info_extract_settings_btn = gr.Button("提取设置", min_width= 1, size ="sm")
                            video_info_to_video_source_btn = gr.Button("设为视频源", min_width= 1, size ="sm", visible = any_video_source)
                            video_info_to_control_video_btn = gr.Button("设为控制视频", min_width= 1, size ="sm", visible = any_control_video )
                            video_info_eject_video_btn = gr.Button("弹出视频", min_width= 1, size ="sm")
                        with gr.Row(**default_visibility_false) as deleted_video_buttons_row:
                            video_info_eject_deleted_video_btn = gr.Button("弹出已删除文件", min_width= 1, size ="sm")
                        with gr.Row(**default_visibility_false) as image_buttons_row:
                            video_info_extract_image_settings_btn = gr.Button("提取设置", min_width= 1, size ="sm")
                            video_info_to_start_image_btn = gr.Button("设为起始图片", size ="sm", min_width= 1, visible = any_start_image )
                            video_info_to_end_image_btn = gr.Button("设为结束图片", size ="sm", min_width= 1, visible = any_end_image)
                            video_info_to_image_mask_btn = gr.Button("设为遮罩图片", min_width= 1, size ="sm", visible = any_image_mask and False)
                            video_info_to_reference_image_btn = gr.Button("设为参考图片", min_width= 1, size ="sm", visible = any_reference_image)
                            video_info_to_image_guide_btn = gr.Button("设为控制图片", min_width= 1, size ="sm", visible = any_control_image )
                            video_info_eject_image_btn = gr.Button("弹出图片", min_width= 1, size ="sm")
                    with gr.Tab("后期处理", id="audio_postprocessing", visible=late_audio_postprocessing_visible) as audio_postprocessing_tab:
                        ...
                        with gr.Row():
                            video_info_audio_postprocessing_btn = gr.Button("应用音频后期处理", size="sm", visible=True)
                            video_info_eject_audio2_btn = gr.Button("弹出音频", size="sm", visible=True)
                    with gr.Tab("后期处理", id= "post_processing", visible = late_video_postprocessing_visible) as video_postprocessing_tab:
                        with gr.Group(elem_classes= "postprocess"):
                            with gr.Column():
                        ...
                        with gr.Row():
                            video_info_postprocessing_btn = gr.Button("应用后期处理", size ="sm", visible=True)
                            video_info_eject_video2_btn = gr.Button("弹出媒体文件", size ="sm", visible=True)
                    with gr.Tab("音频重新封装", id= "audio_remuxing", visible = late_audio_remuxing_visible) as audio_remuxing_tab:
                        ...
                        with gr.Row():
                            video_info_remux_audio_btn = gr.Button("重新封装音频", size ="sm", visible=True)
                            video_info_eject_video3_btn = gr.Button("弹出视频", size ="sm", visible=True)
                    with gr.Tab("导入媒体文件到素材库", id= "video_add"):
                        files_to_load = gr.Files(label= "要导入素材库的媒体文件", height=120)
                        with gr.Row():
                            video_info_add_videos_btn = gr.Button("导入视频 / 图片 / 音频文件", size ="sm")
 
            if not update_form:
                generate_btn = gr.Button("生成")
                add_to_queue_btn = gr.Button("将新提示词加入队列", visible=False)
                ...
                with gr.Column(visible= False) as current_gen_column:
                    with gr.Accordion("预览", open=False):
                        preview = gr.HTML(label="预览", show_label= False)
                        preview_trigger = gr.Text(visible= False)
                    gen_info = gr.HTML(visible=False, min_height=1) 
                    with gr.Row() as current_gen_buttons_row:
                        onemoresample_btn = gr.Button("再生成一个样本", visible = True, size='md', min_width=1)
                        onemorewindow_btn = gr.Button("延长此样本", visible = False, size='md', min_width=1)
                        pause_btn = gr.Button("暂停", visible = True, size='md', min_width=1)
                        resume_btn = gr.Button("恢复", visible = False, size='md', min_width=1)
                        abort_btn = gr.Button("中止", visible = True, size='md', min_width=1)
                        earlystop_btn = gr.Button("提前停止", visible = True, size='md', min_width=1)
                with gr.Accordion("队列管理", open=False) as queue_accordion:
                    with gr.Row():
                        queue_html = gr.HTML(
                    ...
                    with gr.Row(visible= True):
                        ...
                        save_queue_btn = gr.DownloadButton("保存队列", size="sm")
                        load_queue_btn = gr.UploadButton("加载队列", file_types=[".zip", ".json"], size="sm")
                        clear_queue_btn = gr.Button("清空队列", size="sm", variant="stop")
                        quit_button = gr.Button("保存并退出", size="sm", variant="secondary")
                        with gr.Row(visible=False) as quit_confirmation_row:
                            confirm_quit_button = gr.Button("确认", elem_id="comfirm_quit_btn_hidden", size="sm", variant="stop")
                            cancel_quit_button = gr.Button("取消", size="sm", variant="secondary")
...
def get_js():
    ...
    () => {
        setTimeout(() => {
            ...
            const brushButton = document.querySelector('button[aria-label="画笔"]');
            if (brushButton) {
                brushButton.click();
...
def create_ui():
    ...
    with gr.Blocks(css=css, js=js, theme=theme, title="WanGP", fill_width=True) as main:
        gr.Markdown(f"<div align=center><H1>Wan<SUP>GP</SUP> v{WanGP_version} <FONT SIZE=4>作者 <I>DeepBeepMeep</I></FONT> <FONT SIZE=3>") # (<A HREF='https://github.com/deepbeepmeep/Wan2GP'>Updates</A>)</FONT SIZE=3></H1></div>")
        global model_list
        ...
        with gr.Tabs(selected="media_gen", ) as main_tabs:
            ...
            with gr.Tab("媒体生成器", id="media_gen") as media_generator_tab:
                model_toolbar = None
                model_filter = None
                with gr.Row():
                    if args.lock_model:    
            ...
            with gr.Tab("编辑", id="edit", visible=False) as edit_tab:
